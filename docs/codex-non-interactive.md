...
# Orchestration findings (Claude-plans, Codex-codes)
...
There is **no** reasoning-effort CLI flag. `-m`/`--model` sets the model; effort
is a config value passed via `-c key=value` (parsed as TOML). Confirmed key name
...
So the default `codex exec` run uses **gpt-5.5 at medium effort** with no flags.
To override per-invocation:

```bash
codex exec -m gpt-5.5 -c 'model_reasoning_effort="medium"' "<task>"
```

Quote the value so the shell passes valid TOML. Bare `medium` works via the
literal-string fallback but quoting is cleaner.

Validate any config key against your installed version with `--strict-config`,
which errors on unrecognized fields:

```bash
codex exec --strict-config -c 'model_reasoning_effort="medium"' \
  --skip-git-repo-check "say hi"
```

### "fast" is not an effort level

- `gpt-5.5-fast` is a separate **model variant**. On ChatGPT-account auth only
  `gpt-5.5` is reported callable; `-fast`, `-codex`, `-pro` variants require
  API-key auth. Confirm against your own auth before relying on it.
- Priority/speed is otherwise a `service_tier` config value (`-c
  service_tier="..."`), not a model name. `/fast` doubles rate; `flex` ~halves.

## Billing follows auth method, not the subcommand

`codex exec` reuses saved CLI auth by default.

- **ChatGPT sign-in** → usage draws on the ChatGPT/Codex subscription's included
  limits (rolling 5-hour window). No per-token charge.
- **API key** (`CODEX_API_KEY=... codex exec ...`, exec-only) → standard API
  per-token billing on the OpenAI Platform.

For a subscription-funded orchestrator, ensure `codex login status` shows
ChatGPT auth. Caveat for fan-out: parallel `codex exec` workers all draw from
the same 5-hour pool, so N concurrent workers exhaust it ~N× faster. API-key
auth gives per-agent cost attribution if that matters more than subscription
inclusion.

## Pinning Codex to a worktree (critical for parallel steps)

Codex starts **cold** — it inherits no context from the orchestrator. Two
consequences:

1. The plan must be a **self-contained brief** written to a file (exact paths,
   conventions, acceptance criteria, test command, explicit do-not-touch list).
   Pass it by reference, not pasted into the prompt.
2. Codex must be pinned to the correct working directory or it edits the wrong
   tree. For per-step parallel work in isolated git worktrees:

```bash
codex exec -C <worktree-path> --sandbox workspace-write \
  "Implement the plan in planning/milestone-<milestone-id>/<step-id>/plan.md. \
   Read that file first. Follow it exactly. Do not touch anything outside scope."
```

- `-C <dir>` sets the working root.
- `--sandbox workspace-write` is required for Codex to write code (default is
  read-only).
- `--add-dir <dir>` grants additional writable dirs if a step spans more than
  the worktree.
- Codex requires a git repo; a worktree satisfies this. Use
  `--skip-git-repo-check` only for safe non-repo scratch runs.

## Programmatic supervision via JSON stream

For an orchestrator that spawns and monitors Codex, use `--json` (JSONL on
stdout) rather than scraping prose. Key events: `thread.started`,
`turn.started`, `turn.completed` (carries token `usage`), `turn.failed`,
`item.*`, `error`. Extract the final agent message and turn status by event
type, not by line position — event schema field names have drifted between
versions, so inspect a few raw lines on your installed version before writing
parsers.

For typed hand-back to the next pipeline stage, combine `--output-schema
<file>` with `-o <file>` so the final message conforms to a JSON Schema and
lands in a file. This makes Codex workers interchangeable with Claude subagents
from the orchestrator's view.

## Cross-step continuity

`codex exec resume --last` (or `resume <SESSION_ID>`) preserves conversation
context across calls — useful for the review→fix loop on the same step so Codex
remembers what it wrote. Only conversation context carries; re-pass all flags
(`-m`, `-C`, `--sandbox`) each time, they do not persist.

## Claude-side orchestration constraints (not Codex, but load-bearing)

- Subagent nesting requires Claude Code **≥ v2.1.172**. Spawn tree is
...
- Autonomous merge to main is the highest-risk action — gate it behind human
  approval, at least until the loop is trusted.
