...
from fishy.daily_patterns import construct_pattern, pattern_product, annual_magnitude_product, import_pattern
...
def test_replacement_cannot_retain_acceptance_for_changed_hydrograph():
    original = accepted()
    changed = (replace(original.samples[0], value=Flow(9)), replace(original.samples[1],value=Flow(7)), *original.samples[2:])
...
def test_import_cannot_change_receiving_accounting_month():
    ...
    samples=tuple(replace(s, interval=Interval(calendar.interval.start+timedelta(days=i),calendar.interval.start+timedelta(days=i+1))) for i,s in enumerate(original.samples))
    # July 2023 includes leap February: choose July 2022 for a complete 365-day input.
    calendar=AccountingYear(2022,7,0)
    samples=tuple(replace(s, interval=Interval(calendar.interval.start+timedelta(days=i),calendar.interval.start+timedelta(days=i+1))) for i,s in enumerate(original.samples))
    with pytest.raises(ValueError):
        import_pattern(original.magnitude,calendar,samples,original.magnitude.derivation,intended_use="screen",purpose=UsePurpose.SCREENING)
